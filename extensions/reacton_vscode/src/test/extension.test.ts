...
const DIAGNOSTICS_PATTERNS = {
  ...
  readWatch:
    /(?:read|watch|context\.watch|context\.read)\s*\(\s*(\w+)\s*\)/g,
};
...
const GRAPH_LEVELS: Record<string, number> = {
  writable: 0,
  computed: 1,
  selector: 1,
  async: 2,
 ...
 */
function extractDependencies(fullText: string, startLine: number): string[] {
 ...
 */
function findClosingLine(fullText: string, startLine: number): number {
  const lines = fullText.split('\n');
  let depth = 0;
  let started = false;
 ...
 */
function extractCallBody(allLines: string[], startLine: number): string {
  const parts: string[] = [];
  let depth = 0;
  let started = false;
...
/**
 * Replicates _findBuildMethodRanges from diagnostics_provider.ts
 */
...
function findBuildMethodRanges(lines: string[]): BuildMethodRange[] {
...
function isInsideBuildMethod(lineNum: number, buildRanges: BuildMethodRange[]): boolean {
  return buildRanges.some((r) => lineNum > r.startLine && lineNum <= r.endLine);
}

/**
 * Replicates buildDependencyChain from extension.ts
 * Simplified version that works with a Map of reacton-like objects.
 */
interface SimpleReacton {
  name: string;
  type: string;
  valueType: string;
  dependencies: string[];
...
function buildDependencyChain(
  name: string,
  reactonMap: Map<string, SimpleReacton>,
  indent = '',
  ...
  const deps = reacton.dependencies
    ...
    .filter((r): r is SimpleReacton => r !== undefined);
...
let snippets: Record<string, { prefix: string | string[]; body: string[]; description: string }>;
let packageJson: any;
...
describe('Reacton VSCode Extension', function () {
  ...
  describe('Snippet Validation', function () {
    ...
    it('should have all key snippet names', function () {
      ...
      for (const prefix of requiredPrefixes) {
        assert.ok(
          ...
          `Missing required snippet prefix: ${prefix}`
        );
      }
    });
    ...
    for (const [name, snippet] of Object.entries(snippets)) {
      describe(`Snippet: "${name}"`, function () {
        ...
        it('should have at least one prefix starting with "r"', function () {
          ...
          assert.ok(
            hasRPrefix || prefixes.some((p) => p.includes('Reacton') || p.includes('reacton') || p.includes('computed') || p.includes('selector') || p.includes('asyncReacton') || p.includes('family') || p.includes('createEffect') || p.includes('stateMachine') || p.includes('context.') || p.includes('asyncValue') || p.includes('queryAtom') || p.includes('Middleware') || p.includes('AtomModule') || p.includes('formAtom') || p.includes('TestReacton')),
            `Snippet "${name}" should have a prefix starting with "r" or be a Reacton-related keyword`
          );
        });
        ...
        it('should have a non-empty description', function () {
          assert.ok(
            ...
            `Snippet "${name}" should have a non-empty description`
          );
        });
        ...
        it('should not reference old atom() or asyncAtom() API names in body', function () {
          ...
          assert.ok(
            !hasOldAtomCall,
            `Snippet "${name}" should not use old atom() API: ${body}`
          );
          assert.ok(
            !hasOldAsyncAtomCall,
            `Snippet "${name}" should not use old asyncAtom() API: ${body}`
          );
        });
      });
    }
    ...
    it('should have state machine snippet', function () {
      ...
      assert.ok(
        allPrefixes.includes('rstatemachine'),
        'Should have state machine snippet prefix'
      );
    });
  ...
  describe('Package.json Validation', function () {
    ...
    describe('Commands', function () {
      ...
      it('should have titles for all commands', function () {
        ...
        for (const cmd of commands) {
          assert.ok(
            ...
            `Command "${cmd.command}" should have a title`
          );
        }
      });
      ...
      it('should prefix all command titles with "Reacton:"', function () {
        ...
        for (const cmd of commands) {
          assert.ok(
            cmd.title.startsWith('Reacton:'),
            `Command title "${cmd.title}" should start with "Reacton:"`
          );
        }
      });
    });
      ...
      it('should have descriptions for all config properties', function () {
        ...
        for (const [key, value] of Object.entries(properties) as [string, any][]) {
          assert.ok(
            ...
            `Config "${key}" should have a description`
          );
        }
      });
    });
    ...
    describe('Activation Events', function () {
      it('should activate on pubspec.yaml', function () {
        assert.ok(
          packageJson.activationEvents.includes('workspaceContains:pubspec.yaml'),
          'Should activate when workspace contains pubspec.yaml'
        );
      });
    });
    ...
    describe('Keybindings', function () {
      ...
      it('should have keybinding for goToReacton', function () {
        const keybindings = packageJson.contributes.keybindings;
        const goTo = keybindings.find((k: any) => k.command === 'reacton.goToReacton');
      ...
      it('should have keybinding for showGraph', function () {
        const keybindings = packageJson.contributes.keybindings;
        const graph = keybindings.find((k: any) => k.command === 'reacton.showGraph');
      ...
      it('should have "when" clause on all keybindings', function () {
        ...
        for (const binding of keybindings) {
          assert.ok(
            binding.when && binding.when.length > 0,
            `Keybinding for "${binding.command}" should have a "when" clause`
          );
        }
      });
    });
    ...
    describe('Menu Items', function () {
      ...
      it('should have "when" clauses on all editor context menus', function () {
        ...
        for (const item of contextMenus) {
          assert.ok(
            item.when && item.when.length > 0,
            `Menu item for "${item.command}" should have a "when" clause`
          );
          assert.ok(
            item.when.includes('editorLangId == dart'),
            `Menu item for "${item.command}" should require dart language`
          );
        }
      });
      ...
      it('should have "when" clauses on all view/title menus', function () {
        ...
        for (const item of viewMenus) {
          assert.ok(
            item.when && item.when.length > 0,
            `View menu item for "${item.command}" should have a "when" clause`
          );
        }
      });
      ...
      it('should have group properties on context menu items', function () {
        ...
        for (const item of contextMenus) {
          assert.ok(
            item.group && item.group.startsWith('reacton'),
            `Context menu item for "${item.command}" should have a reacton group`
          );
        }
      });
    });
    ...
    describe('Views', function () {
      it('should have reactonExplorer view', function () {
        ...
        const reactonView = explorerViews.find((v: any) => v.id === 'reactonExplorer');
      ...
      it('reactonExplorer should have "when" clause', function () {
        ...
        assert.ok(
          reactonView.when.includes('reacton.isActive'),
          '"when" clause should reference reacton.isActive'
        );
      });
    });
    ...
    describe('Snippets Registration', function () {
      it('should register dart snippets', function () {
        ...
        const dartSnippet = snippetContribs.find((s: any) => s.language === 'dart');
  ...
  describe('Diagnostics Logic', function () {
    ...
    describe('isInsideBuildMethod', function () {
      it('should return true for lines inside build body', function () {
        const ranges: BuildMethodRange[] = [{ startLine: 2, endLine: 5 }];
  ...
  describe('Graph Panel', function () {
    describe('Color mapping', function () {
      ...
      it('should have colors for all 7 types', function () {
        for (const type of expectedTypes) {
          assert.ok(
            GRAPH_COLORS[type],
            `Should have color for type "${type}"`
          );
        }
      });
      ...
      it('should have valid hex color codes', function () {
        ...
        for (const [type, color] of Object.entries(GRAPH_COLORS)) {
          assert.ok(
            ...
            `Color for "${type}" should be valid hex: ${color}`
          );
        }
      });
    ...
    describe('Level mapping', function () {
      ...
      it('should have levels for all 7 types', function () {
        for (const type of expectedTypes) {
          assert.ok(
            type in GRAPH_LEVELS,
            `Should have level for type "${type}"`
          );
        }
      });
  ...
  describe('buildDependencyChain', function () {
    it('should handle unknown reacton', function () {
      const map = new Map<string, SimpleReacton>();
  ...
  describe('Scanner Line Integration', function () {
    function matchScannerLine(line: string): { type: string; name: string; valueType?: string } | null {
